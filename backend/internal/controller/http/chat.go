...
func (r *Router) getChats(ctx *fiber.Ctx) error {
	sortType := ctx.Query("type")
	if !IsValidType(sortType) {
		return ctx.Status(fiber.StatusBadRequest).JSON(model.ErrorResponse("Invalid chat type"))
	}

	chats, total, err := r.chatService.GetAll(ctx.UserContext(), sortType)
	if err != nil {
...
func (r *Router) getChatByID(ctx *fiber.Ctx) error {
	...
	chat, err := r.chatService.GetByID(ctx.UserContext(), id)
	if err != nil {
...
func (r *Router) createChat(ctx *fiber.Ctx) error {
	...
	createdChat, err := r.chatService.Create(ctx.UserContext(), chat)
	if err != nil {
		logger.Log().Error(ctx.UserContext(), err.Error())
		return ctx.Status(fiber.StatusInternalServerError).JSON(model.ErrorResponse(err.Error()))
	}
...
func (r *Router) deleteChat(ctx *fiber.Ctx) error {
	...
	if err := r.chatService.Delete(ctx.UserContext(), id); err != nil {
		logger.Log().Error(ctx.UserContext(), err.Error())
		return ctx.Status(fiber.StatusInternalServerError).JSON(model.ErrorResponse(err.Error()))
	}
